...
const Users = () => {
  ...
  // Using a custom pagination hook to manage paginated user data and current page information.
  const { currentPage, paginatedData, setCurrentPage, pageSize } = usePagination(userData);

  // Function to toggle the delete modal and set the ID of the user to be deleted.
  function showDeleteModel(id) {
    setIdDeleteModal(id);
    setDeleteModalOpen(!deleteModalOpen);  // Toggle the delete modal visibility.
  }

  // Function to open the update modal and set the form data with the user’s current information.
  function showUpdateModel(user: User) {
    setIsUpdateModalOpen(!isUpdateModalOpen);  // Toggle the update modal visibility.
    setUpdateForm({
  ...
  // Function to fetch user data from the server using an API call.
  const fetchUserData = async () => {
    try {
      const response = await urls.getUsers();  // Fetch users from API.
      const data = await response.data;
      setUserData(data);  // Store fetched data in state.
      setItems(data);  // Also store data for search functionality.
    } catch (error) {
      console.error('Error fetching user data:', error);
    }
  };

  // useEffect hook to fetch the user data when the component is first rendered.
  useEffect(() => {
    fetchUserData();
  }, []);  // Empty dependency array means it runs once when the component mounts.

  // Function to handle search input and filter users based on email or full name.
  const handleSearch = (event) => {
    const value = event.target.value.toLowerCase();  // Convert input to lowercase.
    setSearchText(value);

    // If search text is empty, reset the user data to all users.
    if (value.trim() === '') {
      setUserData(items);
    } else {
      // Filter users based on email or full name matching the search text.
      ...
      setUserData(filteredAdmin);  // Update the displayed user data.
    }
  };

  // Function to update the user's data via an API call.
  async function updateUsers() {
    try {
      await urls.updateUsers(updateForm);  // Send updated form data to the server.
      closeUpdateModel();  // Close the update modal after submission.

      // Fetch the updated user data from the server to ensure the UI reflects changes.
      ...
      Notify('Information updated successfully');  // Notify user of successful update.
    } catch (e) {
      console.log('Error:', e);
      ErrorNotify('Error occurred');  // Notify user of an error.
    }
  }

  // Function to handle the form submission for updating users.
  const updateSubmit = async (e: React.FormEvent) => {
    e.preventDefault();  // Prevent the default form submission behavior.
    updateUsers();  // Call the updateUsers function to handle the update.
  };

  // Function to handle form input changes for the update form.
  const updateChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    const { name, value } = e.target;  // Get the name and value of the input field.
    setUpdateForm((prev) => ({
      ...prev,
      [name]: value  // Update the form state dynamically based on input name and value.
    }));
  };

  // Function to delete a user by making an API call.
  async function deleteUsers(id) {
    try {
      await urls.deleteUsers(id);  // Delete the user by ID.

      // Update the user data state to remove the deleted user from the list.
      setUserData((prevData) => prevData.filter(user => user.id !== id));

      setDeleteModalOpen(false);  // Close the delete modal.

      Notify('Information deleted successfully');  // Notify user of successful deletion.
    } catch (e) {
      console.log("Delete failed:", e);
      Notify('Delete failed');  // Notify user of an error in deletion.
    }
  }
